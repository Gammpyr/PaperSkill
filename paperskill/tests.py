...
class CourseModelTest(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            phone_number="+79876543210", password="testpassword123", username="testuser"
        )
    ...
    def test_course_string_representation(self):
        ...
        course = Course.objects.create(name="Тестовый курс", owner=self.user)

        self.assertEqual(str(course), "Тестовый курс [Владелец: +79876543210]")

    def test_course_default_values(self):
        """Тест значений по умолчанию"""
        course = Course.objects.create(name="Курс без описания", owner=self.user)
...
class LessonDetailViewTestCase(TestCase):
    def setUp(self):
        ...
        self.owner = User.objects.create_user(
            phone_number="+79991112233", email="owner@test.com", password="testpass123"
        )
        self.student = User.objects.create_user(
            phone_number="+79992223344", email="student@test.com", password="testpass123"
        )
        self.superuser = User.objects.create_superuser(
            phone_number="+79993334455", email="admin@test.com", password="testpass123"
        )

        # Создаем курс
        self.course_free = Course.objects.create(
            name="Бесплатный курс", description="Описание бесплатного курса", owner=self.owner, is_paid=False
        )

        self.course_paid = Course.objects.create(
            name="Платный курс", description="Описание платного курса", owner=self.owner, is_paid=True, price=1000
        )

        # Создаем уроки
        self.lesson_free = Lesson.objects.create(
            name="Урок в бесплатном курсе", description="Содержание урока", course=self.course_free, owner=self.owner
        )
    ...
    def test_dispatch_free_course_authenticated(self):
        ...
        view, request = self._create_view_with_request(self.student, self.course_free.id, self.lesson_free.id)
    ...
    def test_dispatch_paid_course_student_not_bought(self):
        ...
        view, request = self._create_view_with_request(self.student, self.course_paid.id, self.lesson_paid.id)
    ...
    def test_dispatch_owner_access(self):
        ...
        view, request = self._create_view_with_request(self.owner, self.course_paid.id, self.lesson_paid.id)
    ...
    def test_dispatch_superuser_access(self):
        ...
        view, request = self._create_view_with_request(self.superuser, self.course_paid.id, self.lesson_paid.id)
    ...
    def test_dispatch_lesson_owner_access(self):
        ...
        lesson_owner = User.objects.create_user(
            phone_number="+79994445566", email="lesson_owner@test.com", password="testpass123"
        )

        lesson = Lesson.objects.create(
            name="Урок от другого автора", description="Содержание", course=self.course_paid, owner=lesson_owner
        )

        view, request = self._create_view_with_request(lesson_owner, self.course_paid.id, lesson.id)
    ...
    def test_get_context_data(self):
        ...
        view = self._create_view_with_request_and_object(self.owner, self.course_free, self.lesson_free)
    ...
    def test_can_edit_for_superuser(self):
        ...
        view = self._create_view_with_request_and_object(self.superuser, self.course_free, self.lesson_free)
    ...
    def test_can_edit_for_student(self):
        ...
        view = self._create_view_with_request_and_object(self.student, self.course_free, self.lesson_free)
    ...
    def test_can_edit_for_lesson_owner(self):
        ...
        lesson_owner = User.objects.create_user(
            phone_number="+79995556677", email="lessonowner@test.com", password="testpass123"
        )

        lesson = Lesson.objects.create(
            name="Урок", description="Описание", course=self.course_free, owner=lesson_owner
        )

        view = self._create_view_with_request_and_object(lesson_owner, self.course_free, lesson)
